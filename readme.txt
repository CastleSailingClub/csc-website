...
8a) I've now (7feb2015) done an update so that the contact details for the club 
    ...
    That's because the contact details appear in more than one place in the 
    website, so putting them in one place (contact-info.ini) means that you 
    only need to update the details -once- and the change will be reflected 
    anywhere they appear in the website.  That does mean that, if you write 
    page content that requires contact details to appear, you should use the 
    approach used in contact.php, i.e. "$page->contact" or whatever.
